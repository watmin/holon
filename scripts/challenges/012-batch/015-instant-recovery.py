...
"""
=============================================================================
BATCH 012 - CHALLENGE 015: Instant Recovery Detection
=============================================================================

Insight: The 4% FP is ONLY window carryover. Rate is back to normal (raw=0).

Fix: When rate z-score indicates clearly normal, instantly exit anomaly state.
No need to wait for window to clear.

Run: ./scripts/run_with_venv.sh python scripts/challenges/012-batch/015-instant-recovery.py
"""
...
class InstantRecoveryDetector:
    """
    Instant recovery when rate confirms normal.

    Key insight: If rate z > 0 (clearly in normal range),
    we can instantly exit anomaly state regardless of window.
    """
    ...
    def __init__(self, warmup_packets: int = WARMUP_PACKETS):
        ...
        self.anomaly_window = deque(maxlen=10)  # Shorter window
    ...
    def process(self, packet: dict, pps: float) -> dict:
        ...
        # If rate is clearly in normal range (z > 0), immediately exit anomaly
        rate_clearly_normal = rate_z > 0

        if rate_clearly_normal:
            # Instant exit from anomaly state
            self.in_anomaly_state = False
            self.anomaly_window.clear()
            is_anomalous = False
        else:
            # Normal window-based detection
            self.anomaly_window.append(1 if raw_anomaly else 0)

            if len(self.anomaly_window) == 0:
                fraction = 0.0
            else:
                fraction = sum(self.anomaly_window) / len(self.anomaly_window)
        ...
        return {
            ...
            "pattern_z": pattern_z,
            "raw": raw_anomaly,
            "rate_normal": rate_clearly_normal,
        }
...
def run_test(attack_type: str, phases: List[TimePhase], scale: float = 0.005):
    print(f"\n{'='*70}")
    print(f"INSTANT RECOVERY TEST: {attack_type}")
    print(f"{'='*70}")
    ...
    detector = InstantRecoveryDetector(warmup_packets=warmup_packets)
    ...
    for phase in phases:
        ...
        for i in range(scaled_packets):
            ...
            if detector.warmup_complete:
                ...
                if result.get("raw", False):
                    raw_triggers += 1
                if result.get("rate_normal", False):
                    rate_normal_count += 1
        ...
        if not detector.warmup_complete or phase.name == "calm-1":
            ...
            if phase.phase_type == Phase.ATTACK:
                ...
                status = "CLEAN" if detection_rate < 0.01 else "FP"
    ...
    for pr in phase_results:
        if pr["status"] == "WARMUP":
            ...
            print(f"  {pr['name']:12} ({pr['packets']:5} pkts): {phase_type} "
                  f"Det={pr['detection_rate']:.0%} (raw:{pr['raw_triggers']}, normal:{pr['rate_normal']}) "
                  f"{marker} {pr['status']}")
...
def main():
    print("="*70)
    print("BATCH 012 - CHALLENGE 015: Instant Recovery Detection")
    print("="*70)
    print("""
    Key insight: Rate z-score IMMEDIATELY tells us if traffic is normal.

    Logic:
    - rate_z > 0 → DEFINITELY normal, instant exit from anomaly
    - rate_z < -2.5 → DEFINITELY attack
    - In between → use window smoothing

    This eliminates window carryover FPs because as soon as
    traffic returns to normal rate, we exit anomaly state.
    """)
...
===========
  ...
  014: 100% recall,  4% FP  (fast recovery)
  015: {avg_recall:.0%} recall, {avg_fp:.0%} FP  (instant recovery)
    """)
