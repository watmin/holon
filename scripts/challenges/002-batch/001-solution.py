...
def generate_rpm_matrix(matrix_id, rule_type, attributes=None, missing_position=None):
    ...
    if rule_type == 'progression':
        ...
        row_shapes = [{'circle', 'square'}, {'square', 'triangle'}, {'triangle', 'diamond'}]
        col_shapes = [{'circle', 'triangle'}, {'square', 'diamond'}, {'circle', 'square'}]
...
def create_synthetic_matrices():
    ...
    matrices.append(generate_rpm_matrix("rpm-shape-only-1", "progression",
                                       {'shape'}, "row2-col2"))

    # Matrix with only count attribute
    matrices.append(generate_rpm_matrix("rpm-count-only-1", "union",
                                       {'count'}, None))
...
def query_matrices(store, query, description, top_k=5, guard=None, negations=None):
    ...
    try:
        results = store.query(query, guard=guard, negations=negations,
                            top_k=top_k, threshold=0.0, data_type='json')

        if not results:
            print("  ❌ No matching matrices found")
            return

        print(f"  ✅ Found {len(results)} matching matrices (showing top {min(top_k, len(results))}):")
        ...
        for i, (matrix_id, score, matrix_data) in enumerate(results):
            ...
            for pos in sample_panels:
                ...
                print(f"     {pos}: shapes={shapes}, count={panel.get('count', 0)}, color={panel.get('color', 'unknown')}")
...
def compute_expected_missing_panel(matrix_data, missing_position):
    ...
    if rule == 'progression':
        ...
        expected_panel = {'shapes': set(), 'count': 0, 'color': 'unknown', 'rule': 'unknown'}
...
def demonstrate_missing_panel_completion(store):
    ...
    incomplete_results = store.query('{"missing-position": "row3-col3"}',
                                   top_k=3, data_type='json')
    ...
    for i, (matrix_id, score, matrix_data) in enumerate(incomplete_results):
        ...
        print(f"     Shapes: {list(expected_panel['shapes'])} (count: {expected_panel['count']})")
        ...
        for pos in ['row1-col1', 'row1-col2', 'row1-col3', 'row2-col1', 'row2-col2', 'row2-col3', 'row3-col1', 'row3-col2']:
            if pos in panels:
                panel = panels[pos]
                shapes = panel.get('shapes', [])
                print(f"     {pos}: {list(shapes)} (count: {len(shapes)}, color: {panel.get('color', 'unknown')})")
        ...
        complete_results = store.query(edn_to_json(probe_structure),
        ...
        for j, (comp_id, comp_score, comp_data) in enumerate(complete_results):
            ...
            is_correct = (expected_shapes == actual_shapes and
            ...
            print(f"        Found: {list(actual_shapes)} (count: {len(actual_shapes)}, color: {actual_missing.get('color', 'unknown')})")

        if found_correct:
            print("   🎯 SUCCESS: Geometric similarity found the correct missing panel!")
        else:
            print("   ⚠️  Note: No exact match found (could be due to different matrix generation parameters)")
