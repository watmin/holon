...
def generate_synthetic_graphs():
    ...
    add_graph("triangle_cycle", ["A", "B", "C"],
              [("A", "B"), ("B", "C"), ("C", "A")],
              "undirected", {"A": {"color": "red"}, "B": {"color": "blue"}, "C": {"color": "green"}},
              "3-node cycle (triangle)")

    add_graph("square_cycle", ["A", "B", "C", "D"],
              [("A", "B"), ("B", "C"), ("C", "D"), ("D", "A")],
              "undirected", {},
              "4-node cycle (square)")

    add_graph("pentagon_cycle", ["A", "B", "C", "D", "E"],
              [("A", "B"), ("B", "C"), ("C", "D"), ("D", "E"), ("E", "A")],
              "undirected", {},
              "5-node cycle (pentagon)")

    # 6-10: Tree structures
    add_graph("star_tree", ["Center", "Leaf1", "Leaf2", "Leaf3", "Leaf4"],
              [("Center", "Leaf1"), ("Center", "Leaf2"), ("Center", "Leaf3"), ("Center", "Leaf4")],
              "undirected", {"Center": {"role": "hub"}},
              "Star topology (center connected to all leaves)")

    add_graph("binary_tree", ["Root", "L1", "R1", "L2", "R2"],
              [("Root", "L1"), ("Root", "R1"), ("L1", "L2"), ("R1", "R2")],
              "undirected", {},
              "Perfect binary tree")

    add_graph("chain_tree", ["A", "B", "C", "D", "E"],
              [("A", "B"), ("B", "C"), ("C", "D"), ("D", "E")],
              "undirected", {},
              "Linear chain (path graph)")

    # 11-15: Random graphs with varied connectivity
    add_graph("sparse_random", ["A", "B", "C", "D", "E", "F"],
              [("A", "B"), ("A", "C"), ("B", "D"), ("C", "E"), ("D", "F")],
              "undirected", {},
              "Sparse random graph (tree-like)")

    add_graph("dense_random", ["A", "B", "C", "D", "E"],
              [("A", "B"), ("A", "C"), ("A", "D"), ("A", "E"),
               ("B", "C"), ("B", "D"), ("C", "D"), ("C", "E"), ("D", "E")],
              "undirected", {},
              "Dense random graph (nearly complete)")

    add_graph("hub_spoke", ["Hub", "S1", "S2", "S3", "S4", "S5", "S6"],
              [("Hub", "S1"), ("Hub", "S2"), ("Hub", "S3"), ("Hub", "S4"),
               ("Hub", "S5"), ("Hub", "S6")],
              "undirected", {"Hub": {"type": "central"}},
              "Hub-and-spoke topology")

    # 16-20: Directed graphs and special cases
    add_graph("directed_cycle", ["A", "B", "C", "D"],
              [("A", "B", "flows_to"), ("B", "C", "flows_to"),
               ("C", "D", "flows_to"), ("D", "A", "flows_to")],
              "directed", {},
              "Directed cycle")

    add_graph("workflow", ["Start", "Process", "Review", "Approve", "End"],
              [("Start", "Process", "starts"), ("Process", "Review", "submits"),
               ("Review", "Approve", "recommends"), ("Approve", "End", "completes")],
              "directed", {},
              "Workflow process graph")

    add_graph("hierarchy", ["CEO", "VP1", "VP2", "Mgr1", "Mgr2", "Emp1", "Emp2"],
              [("CEO", "VP1", "manages"), ("CEO", "VP2", "manages"),
               ("VP1", "Mgr1", "manages"), ("VP2", "Mgr2", "manages"),
               ("Mgr1", "Emp1", "manages"), ("Mgr2", "Emp2", "manages")],
              "directed", {},
              "Organizational hierarchy")

    add_graph("web_graph", ["Home", "About", "Products", "Contact", "Blog"],
              [("Home", "About", "links"), ("Home", "Products", "links"),
               ("Home", "Contact", "links"), ("About", "Blog", "links"),
               ("Products", "Contact", "links"), ("Blog", "Products", "links")],
              "directed", {},
              "Website navigation graph")

    add_graph("social_network", ["Alice", "Bob", "Charlie", "Diana", "Eve"],
              [("Alice", "Bob", "friends"), ("Alice", "Charlie", "friends"),
               ("Bob", "Diana", "friends"), ("Charlie", "Diana", "friends"),
               ("Charlie", "Eve", "friends"), ("Diana", "Eve", "friends")],
              "undirected", {"Alice": {"influence": 8}, "Bob": {"influence": 6}},
...
def query_graphs(store, query, description, top_k=10, guard=None, negations=None, threshold=0.0):
    ...
    try:
        results = store.query(query, data_type='edn', guard=guard, negations=negations,
        ...
        print(f"  ✅ Found {len(results)} matching graphs (showing top {min(top_k, len(results))}):")
        ...
        for i, (graph_id, score, graph_data) in enumerate(results):
            ...
            print(f"\n  {i+1}. [{score:.3f}] {name}")
            print(f"     Type: {graph_type} | Nodes: {len(nodes)} | Edges: {len(edges)}")
            if desc:
...
def main():
    ...
    print("   • Star topology gets highest similarity to itself (perfect geometric alignment)")
    print("   • Other tree structures show moderate similarity (shared hierarchical properties)")
    ...
    print("   • 'Close enough' approximations for most practical graph similarity tasks!")

    print("\n" + "=" * 55)
    print("🎉 Approximate Graph Matching Demo Complete!")
    print("Holon successfully demonstrated geometric approximate graph similarity and subgraph matching")
    print("High similarity scores indicate geometric proximity ('close enough' structural matching)")
    print("=" * 55)
