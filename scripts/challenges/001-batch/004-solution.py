...
class MagicMemoryStore:
    ...
    def find_similar_magic(self, probe: Dict[str, Any], top_k: int = 10,
        ...
        return [(bug_id, score, self._get_magic_item(bug_id)) for bug_id, score, _ in results]

    def query_magic(self, probe: Dict[str, Any] = None,
        ...
        return [(magic_id, score, self._get_magic_item(magic_id))
...
def generate_dnd_spells() -> List[Dict[str, Any]]:
    ...
    spells = [
        ...
        },
        {
            ...
            'level': 2,
            'school': ':enchantment',
            'classes': {':sorcerer', ':wizard', ':bard', ':cleric', ':druid', ':warlock'},
            'casting_time': '1 action',
            'range': '60 feet',
            'components': {':v', ':s', ':m'},
...
def demonstrate_spell_queries(magic_store: MagicMemoryStore):
    ...
    negations = {
        'duration': {'$contains': 'concentration'}  # Not concentration spells
    }
    ...
    guard = {
        'level': 'cantrip',
        'tags': {'$contains': 'stealth'}
    }

    results = magic_store.query_magic(guard=guard, top_k=10)
    stealth_cantrips = [(mid, score, magic) for mid, score, magic in results
                       if 'illusion' in magic.get('tags', set()) or 'stealth' in magic.get('tags', set())]

    print(f"Found {len(stealth_cantrips)} stealth/illusion cantrips:")
    for i, (magic_id, score, magic) in enumerate(stealth_cantrips[:3], 1):
        print(f"  {i}. {magic['name']} ({magic['school']}) - tags: {', '.join(magic['tags'])}")
    print()
    ...
    results = magic_store.query_magic(guard=guard, top_k=10)
    flight_items = [(mid, score, magic) for mid, score, magic in results
                   if 'flight' in magic.get('tags', set()) or 'teleportation' in magic.get('tags', set())]

    print(f"Found {len(flight_items)} non-legendary flight/teleport items:")
    for i, (magic_id, score, magic) in enumerate(flight_items, 1):
        print(f"  {i}. {magic['name']} ({magic['rarity']}) - {', '.join(magic['tags'])}")
    print()
    ...
    for i, (magic_id, score, magic) in enumerate(results, 1):
        print(f"  {i}. {magic['name']} ({magic.get('type', 'unknown')}) - {', '.join(magic.get('tags', []))}")
    print()
