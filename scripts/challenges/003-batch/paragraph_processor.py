...
class ParagraphProcessor:
    ...
    def process_document(self) -> List[Dict[str, Any]]:
        ...
        print(f"📖 Processing {'PDF' if self.is_pdf else 'text'} document: {self.file_path}")
    ...
    def save_paragraphs_to_json(self, paragraphs: List[Dict[str, Any]], output_path: str):
...
def main():
    ...
    try:
        ...
        print(f"   Chapters found: {set(p['coordinates']['chapter'] for p in paragraphs)}")
        print(f"   Page range: {min(p['coordinates']['page_start'] for p in paragraphs)} - {max(p['coordinates']['page_end'] for p in paragraphs)}")
        print(f"   Avg words per paragraph: {sum(p['coordinates']['word_count'] for p in paragraphs) // len(paragraphs)}")
        ...
        for i, p in enumerate(paragraphs[:3]):
            coord = p['coordinates']
            print(f"   {p['id']}: {coord['chapter']} | Para {coord['paragraph_num']} | Page {coord['page_start']}")
