...
class QuoteProcessor:
    ...
    def _extract_from_text(self) -> List[Dict[str, Any]]:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if quote_match:
                ...
                print(
                    f"📝 Quote {len(quotes)}: [{chapter}] Page {page} | {quote_text[:50]}..."
                )
    ...
    def _extract_from_pdf(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                raise ImportError(
                    "PDF processing requires pypdf or PyPDF2. Install with: pip install pypdf"
                )
        ...
        with open(self.file_path, "rb") as file:
            ...
            for page_num, page in enumerate(pdf_reader.pages, 1):
                ...
                pages_data.append(
                    {"page": page_num, "text": text, "word_count": len(text.split())}
                )
        ...
        for page_data in pages_data:
            page_quotes = self._extract_quotes_from_page_text(
                page_data["text"], page_data["page"]
            )
    ...
    def _extract_quotes_from_page_text(
        self, page_text: str, page_num: int
    ) -> List[Dict[str, Any]]:
        ...
        for quote_text in matches:
            ...
            context_end = min(
                len(page_text),
                page_text.find(f'"{quote_text}"') + len(quote_text) + 200,
            )
            context = (
                page_text[context_start:context_end]
                .replace(f'"{quote_text}"', "[QUOTE]")
                .strip()
            )
            ...
            quotes.append(quote_data)
            print(
                f"📝 Quote {len(quotes)}: [{chapter}] Page {page_num} | {quote_text[:50]}..."
            )
    ...
    def _extract_chapter_from_context(self, context: str) -> str:
        ...
        chapter_patterns = [r"chapter\s+(\w+)", r"Chapter\s+(\w+)", r"CHAPTER\s+(\w+)"]
            ...
            json.dump(
                {
                    "metadata": {
                        ...
                        "extraction_method": "pdf_quote_extraction"
                        if self.is_pdf
                        else "text_quote_extraction",
...
def main():
    ...
    if pdf_file.exists():
        ...
        print(
            "Please ensure either calculus-made-easy.pdf or quotes.txt is in docs/challenges/003-batch/"
        )
        return
    ...
    try:
        ...
        print(
            f"   Page range: {min(q['page'] for q in quotes)} - {max(q['page'] for q in quotes)}"
        )
        print(
            f"   Word count range: {min(q['word_count'] for q in quotes)} - {max(q['word_count'] for q in quotes)}"
        )
        print(f"   Output saved to: {output_json}")

        if processor.is_pdf and len(quotes) == 0:
            print(
                "⚠️  No quotes found in PDF. The PDF might not contain properly formatted quotes."
            )
