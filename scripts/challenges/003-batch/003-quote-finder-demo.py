...
class QuoteFinder:
    ...
    def search_quotes(self, query_phrase: str, top_k: int = 5, threshold: float = 0.1,
                     chapter_filter: str = None) -> List[Dict[str, Any]]:
        ...
        if results:
            print(f"DEBUG: First result score: {results[0][1]:.3f}, metadata: {results[0][2]['metadata']}")
...
def main():
    ...
    for result in results:
        print(f"   Score: {result['score']:.3f} | Chapter: {result['metadata']['chapter']} | Page: {result['metadata']['page']}")
    ...
    results = finder.search_quotes("depends on relative smallness", threshold=0.0)  # Lower threshold for fuzzy
    print(f"   Found {len(results)} results:")
    for result in results:
        print(f"   Score: {result['score']:.3f} | Chapter: {result['metadata']['chapter']} | Page: {result['metadata']['page']}")
        print(f"   Text: {result['reconstructed_text'][:80]}...")
    ...
    results = finder.search_quotes("differential symbol", threshold=0.0)  # Lower threshold for partial
    print(f"   Found {len(results)} results:")
    for result in results:
        print(f"   Score: {result['score']:.3f} | Chapter: {result['metadata']['chapter']} | Page: {result['metadata']['page']}")
        print(f"   Text: {result['reconstructed_text'][:80]}...")
    ...
    results = finder.search_quotes("integration", threshold=0.0, chapter_filter="Unknown")  # All are Unknown, search for integration
    print(f"   Found {len(results)} results:")
    for result in results:
        print(f"   Score: {result['score']:.3f} | Chapter: {result['metadata']['chapter']} | Page: {result['metadata']['page']}")
        print(f"   Text: {result['reconstructed_text'][:80]}...")
