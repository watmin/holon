...
def generate_extreme_data(num_items=1000):
    ...
    for i in range(num_items):
        blob = {
            ...
            "meta": {"timestamp": i * 1000, "value": random.random()},
        }
        data.append(blob)
    return data
...
def insert_data_http(base_url, data, max_workers=10):
    ...
    def insert_item(item):
        response = requests.post(
            f"{base_url}/insert", json={"data": json.dumps(item), "data_type": "json"}
        )
...
def main():
    ...
    try:
        ...
        print(
            f"  Insert Time: {insert_time:.2f}s ({num_items/insert_time:.1f} items/sec)"
        )
        if not success:
