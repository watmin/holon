...
def generate_complex_queries(num_queries=500):
    ...
    for i in range(num_queries):
        query_type = random.choice(
            ["fuzzy", "wildcard", "guard", "negation", "or", "combined"]
        )
...
def run_extreme_challenge():
    ...
    print(
        f"🚀 Holon Extreme Query Challenge: {num_items} Blobs, {num_queries} Complex Queries"
    )
    print("=" * 70)
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [
            executor.submit(run_query, i, probe, guard, negations)
