...
def validate_progression_matrix():
    ...
    matrix = generate_rpm_matrix("test-progression", "progression",
                               {'shape', 'count', 'color'}, "row3-col3")

    print("Generated progression matrix:")
    for pos in ['row1-col1', 'row1-col2', 'row1-col3',
        ...
        print(f"  {pos}: {shapes} (count: {count}, expected: {expected_count}) {status}")
    ...
    for i in range(expected_count):
        if i < len(['circle', 'square', 'triangle', 'diamond', 'star']):
            expected_missing.append(['circle', 'square', 'triangle', 'diamond', 'star'][i])
...
def validate_xor_matrix():
    ...
    matrix = generate_rpm_matrix("test-xor", "xor",
                               {'shape', 'count', 'color'}, "row3-col3")
    ...
    for pos in ['row1-col1', 'row1-col2', 'row1-col3',
        ...
        expected_shapes = []
        if xor_result & 1: expected_shapes.append('circle')    # bit 0
        if xor_result & 2: expected_shapes.append('square')    # bit 1
        if xor_result & 4: expected_shapes.append('triangle')  # bit 2

        is_correct = set(shapes) == set(expected_shapes)
        status = "✅" if is_correct else "❌"
        print(f"  {pos}: {shapes} (xor={xor_result:02b}, expected: {expected_shapes}) {status}")

    # Check missing panel prediction
    row, col = 3, 3
    xor_result = row ^ col  # 3 ^ 3 = 0
    expected_missing = []
    if xor_result & 1: expected_missing.append('circle')
    if xor_result & 2: expected_missing.append('square')
    if xor_result & 4: expected_missing.append('triangle')
...
def test_generation_consistency():
    ...
    matrices = []
    for i in range(5):
        matrix = generate_rpm_matrix(f"consistency-{i}", "xor",
                                   {'shape', 'count', 'color'})
    ...
    for i, matrix in enumerate(matrices):
        ...
        if first_result is None:
            ...
            consistent = False
            print(f"  ❌ Inconsistency at matrix {i}: {shapes} (count: {count}) vs {first_result[0]} (count: {first_result[1]})")

    if consistent:
        print(f"  ✅ All matrices consistent: {first_result[0]} (count: {first_result[1]})")
