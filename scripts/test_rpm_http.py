...
def start_server():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("holon_server", "scripts/holon_server.py")
...
def insert_matrix(matrix_data):
    ...
    try:
        response = client.post(
            "/insert",
            json={"data": edn_to_json(matrix_data)}
        )
        if response.status_code == 200:
...
def test_rpm_http_solution():
    ...
    try:
        ...
        matrices = [
            generate_rpm_matrix("http-xor-test-1", "xor", {'shape', 'count', 'color'}, "row3-col3"),
            generate_rpm_matrix("http-progression-test-1", "progression", {'shape', 'count', 'color'}, "row3-col3"),
            generate_rpm_matrix("http-union-test-1", "union", {'shape', 'count', 'color'}, "row3-col3"),
            # Complete matrices for reference
            generate_rpm_matrix("http-xor-complete-1", "xor", {'shape', 'count', 'color'}),
            generate_rpm_matrix("http-progression-complete-1", "progression", {'shape', 'count', 'color'}),
        ]
        ...
        if query_result:
            ...
            for result in results[:2]:  # Show first 2
                ...
                print(f"   - {data.get('matrix-id', 'unknown')} ({data.get('rule', 'unknown')} rule)")
        ...
        if missing_pos:
            print(f"   Testing completion for {incomplete_xor['matrix-id']} (missing {missing_pos})")
            ...
            probe_structure = {
                "panels": {pos: panel for pos, panel in incomplete_xor.get('panels', {}).items()
                          if pos != missing_pos},
                "rule": incomplete_xor.get('rule', '')
            }

            query_result = query_matrices(edn_to_json(probe_structure),
                                         negations={"missing-position": {"$any": True}},
                                         top_k=3)

            if query_result:
                results = query_result.get("results", [])
                print(f"   HTTP query found {len(results)} geometrically similar complete matrices:")
                ...
                if found_correct:
                    print("   🎯 SUCCESS: HTTP API found geometrically correct missing panel!")
                else:
