...
def test_debug_approach():
    ...
    complete_matrices = [
        generate_rpm_matrix("debug-progression", "progression", {'shape', 'count', 'color'}),
    ...
    incomplete_matrix = generate_rpm_matrix("debug-incomplete", "xor",
    ...
    print(f"Expected missing panel: {list(expected_panel['shapes'])} (count: {expected_panel['count']})")
    ...
    for result in results:
        ...
        is_correct = (actual_shapes == expected_panel['shapes'] and
                     actual_count == expected_panel['count'])
...
def test_comprehensive_approach():
    ...
    for i in range(5):
        ...
        complete = generate_rpm_matrix(f"comp-complete-{i}", "xor", {'shape', 'count', 'color'})
        matrix_json = edn_to_json(complete)
        store.insert(matrix_json)

        # Add incomplete matrix
        incomplete = generate_rpm_matrix(f"comp-incomplete-{i}", "xor",
                                       {'shape', 'count', 'color'}, "row3-col3")
        matrix_json = edn_to_json(incomplete)
        store.insert(matrix_json)

    # Test one completion
    test_matrix = generate_rpm_matrix("comp-test", "xor", {'shape', 'count', 'color'}, "row3-col3")
    expected_panel = compute_expected_missing_panel(test_matrix, "row3-col3")
    print(f"Expected missing panel: {list(expected_panel['shapes'])} (count: {expected_panel['count']})")

    probe_structure = {
        "panels": test_matrix['panels'],
        "rule": "xor"
    }
...
def main():
    ...
    print(f"Debug approach (simple matrices): {'✅ WORKS' if debug_works else '❌ FAILS'}")
    print(f"Comprehensive approach (many matrices): {'✅ WORKS' if comprehensive_works else '❌ FAILS'}")
    ...
    if debug_works and not comprehensive_works:
        ...
        print("💡 Issue: Vector similarity overwhelmed by noise from multiple similar matrices")
