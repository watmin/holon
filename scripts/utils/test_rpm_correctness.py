...
def test_rule_correctness(store, rule_type, test_cases=5):
    ...
    for i in range(test_cases):
        ...
        matrix = generate_rpm_matrix(
            matrix_id, rule_type, {"shape", "count", "color"}, "row3-col3"
        )
        ...
        print(
            f"  Test {i+1}: {status} | Expected: {list(expected_panel['shapes'])} "
...
def test_rule_discrimination(store):
    ...
    discrimination_score = sum(score for score in test_results.values()) / len(
        test_results
    )
...
def test_wrong_rule_failure(store):
    ...
    for result in results:
        ...
        if expected_shapes == actual_shapes and expected_panel[
            "color"
        ] == actual_missing.get("color", ""):
            wrong_rule_success = True
            break

    if not wrong_rule_success:
        print(
            "  ✅ Wrong rule correctly failed - system didn't find progression result when querying as XOR"
        )
        return True
    else:
        print(
            "  ❌ Wrong rule incorrectly succeeded - system found progression result for XOR query"
        )
        return False
