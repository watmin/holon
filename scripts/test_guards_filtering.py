...
data = [
    ...
    {"user": "alice", "action": "login", "status": "failed", "ip": "192.168.1.2"},  # Similar but failed
    {"user": "alice", "action": "logout", "status": "success", "ip": "192.168.1.1"},  # Different action
    {"user": "bob", "action": "login", "status": "success", "ip": "10.0.0.1"},  # Different user
    {"user": "alice", "action": "login", "status": "success", "ip": "192.168.1.3", "extra": "data"},  # Extra field
]
...
print(f"Failed logins in guarded results: {len([r for r in results_guarded if r[2]['status'] == 'failed'])}")
