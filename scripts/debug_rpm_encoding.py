...
def debug_matrix_encoding():
    ...
    test_matrices = [
        {
            ...
            'panels': {
                ...
                'row2-col1': {'shapes': ['circle', 'square'], 'count': 2, 'color': 'black'},
                'row2-col2': {'shapes': ['circle', 'square', 'triangle'], 'count': 3, 'color': 'white'},
                'row2-col3': {'shapes': ['circle', 'square', 'triangle', 'diamond'], 'count': 4, 'color': 'red'},
                'row3-col1': {'shapes': ['circle', 'square', 'triangle'], 'count': 3, 'color': 'black'},
                'row3-col2': {'shapes': ['circle', 'square', 'triangle', 'diamond'], 'count': 4, 'color': 'white'},
                'row3-col3': {'shapes': ['circle', 'square', 'triangle', 'diamond', 'star'], 'count': 5, 'color': 'red'}
            }
        },
        {
    ...
    for matrix in test_matrices:
        ...
        for pos in ['row1-col1', 'row1-col2', 'row1-col3',
            ...
            if pos in matrix['panels']:
                ...
                print(f"    {pos}: {shapes} (count: {len(shapes)}, color: {panel.get('color', 'unknown')})")
    ...
    results = store.query(json.dumps(probe, default=list),
    ...
    for i, (id, score, data) in enumerate(results):
        ...
        is_correct = (actual_shapes == [] and actual_count == 0 and actual_color == 'black')
    ...
    if found_correct:
        ...
        complete_results = store.query('{"rule": "xor"}', negations={"missing-position": {"$any": True}})
        if complete_results:
            data = complete_results[0][2]
            complete_panel = data.get('panels', {}).get('row3-col3', {})
            print(f"  Expected from complete matrix: {complete_panel.get('shapes', [])} "
