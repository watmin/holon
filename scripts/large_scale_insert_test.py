...
def insert_batch_http(batch: List[dict], batch_num: int) -> tuple:
    ...
    response = requests.post(f"{BASE_URL}/batch_insert", json={
        "items": [json.dumps(item) for item in batch],
        "data_type": "json"
    })
...
def main():
    ...
    batches = [all_items[i:i + OPTIMAL_BATCH_SIZE] for i in range(0, len(all_items), OPTIMAL_BATCH_SIZE)]
    ...
    print("\n💡 Performance Analysis:")
    print(f"- Pure insert time: {total_time:.3f}s ({total_time/total_wall_time*100:.1f}% of wall time)")
    ...
    # Scaling projections
    print("\n📈 Scaling Projections:")
    print(f"- 100k items: ~{total_wall_time * 2:.1f}s ({TARGET_TOTAL * 2 / total_wall_time:.0f}/sec)")
    print(f"- 1M items: ~{total_wall_time * 20:.1f}s ({TARGET_TOTAL * 20 / total_wall_time:.0f}/sec)")
