...
def log_system_stats():
    ...
    logger.info(f"MEMORY: {mem.used/1024/1024:.1f}MB used, {mem.available/1024/1024:.1f}MB available, {mem.percent:.1f}%")
...
def log_performance_metrics(phase, start_time, items_processed=0, queries_processed=0):
    ...
    if items_processed > 0:
        ...
        logger.info(f"PHASE_{phase}: Processed {items_processed} items in {elapsed:.1f}s ({rate:.1f} items/sec)")
    elif queries_processed > 0:
        rate = queries_processed / elapsed
        logger.info(f"PHASE_{phase}: Processed {queries_processed} queries in {elapsed:.1f}s ({rate:.1f} queries/sec)")
    else:
        ...
        complex_item = {
                ...
                "user": {
                    "profile": {
                        ...
                        "professional": {
                            ...
                            "experience": i % 30,
                            "certifications": [{"name": f"cert_{k}", "year": 2020 + k} for k in range(i % 5)]
                        }
                    },
                    "preferences": {
            ...
            "data": {
                ...
                "nested_maps": {
                    f"level1_{x}": {
                        f"level2_{y}": {
                            f"level3_{z}": f"value_{x}_{y}_{z}"
                            for z in range(3)
                        }
                        for y in range(3)
                    }
...
def encode_item_batch(batch_data):
    ...
    for item in items:
        try:
            parsed = json_fast.loads(item) if 'json_fast' in globals() else json.loads(item)
...
def extreme_stress_test():
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        ...
        for batch_start in range(0, len(data), batch_size):
            ...
            batch_data = (batch, {})  # Start with empty global atoms, processes will build locally
        ...
        for future in as_completed(futures):
            try:
                encoded_results, local_atoms = future.result(timeout=300)  # 5min timeout
            except Exception as e:
            ...
            if completed_batches % 10 == 0 or completed_batches >= len(futures):
                ...
                print(f"  Completed {completed_batches}/{len(futures)} batches, inserted {current_count:,} items - {rate:.1f} items/sec - Memory: {current_mem:.1f} GB")
                logger.info(f"BATCH_PROGRESS: {completed_batches}/{len(futures)} batches, {current_count} items, {rate:.1f} items/sec, {current_mem:.1f}GB memory")
                log_system_stats()
    ...
    logger.info(f"Inserted {len(ids)} items with {n_workers} parallel workers, final memory {final_mem:.1f}GB")
    log_system_stats()
    # Generate queries
    ...
    query_chunks = [queries[i:i+queries_per_worker] for i in range(0, n_queries, queries_per_worker)]
    ...
    # Analyze results
    successful_queries = [r for r in results if not isinstance(r[2], str) or not r[2].startswith("ERROR")]
    error_queries = [r for r in results if isinstance(r[2], str) and r[2].startswith("ERROR")]
    avg_results = sum(r[2] for r in successful_queries) / len(successful_queries) if successful_queries else 0
    avg_time = sum(r[3] for r in results) / len(results)

    total_time = time.time() - start_total

    logger.info("TEST COMPLETE")
    logger.info(f"Final stats: {len(ids)} items, {total_time:.1f}s runtime, {final_mem:.1f}GB memory")
    logger.info(f"Query performance: {len(successful_queries)}/{len(results)} successful, {avg_time:.4f}s avg time")
    ...
    print("Check the detailed log file: extreme_stress_test.log for performance analysis!")
